...
%The example for LUAD data.
clear;
clc;
close all;
[profile,pipi]= xlsread('LUAD_data.xlsx');
[empty,network]= xlsread('network.xlsx');


normal=profile(:,2:60);
case_mprofile=profile(:,61:563);
edge_source=network(:,1);
edge_target=network(:,2);
node1=unique(network(:,1));
reference_sample_num=59;
case_mprofile=fillmissing(case_mprofile,'constant',0);
es=0.00000001;
patients_num=[273,50,71,73,11,25];


tempcase(:,1,1:patients_num(1))=case_mprofile(:,1:273);   % Stage I
tempcase(:,2,1:patients_num(2))=case_mprofile(:,274:323);   % Stage IIA
tempcase(:,3,1:patients_num(3))=case_mprofile(:,324:394);   % Stage IIB
tempcase(:,4,1:patients_num(4))=case_mprofile(:,395:467);  % Stage IIIA
tempcase(:,5,1:patients_num(5))=case_mprofile(:,468:478); % Stage IIIB
tempcase(:,6,1:patients_num(6))=case_mprofile(:,479:503); % Stage IV


psize=size(tempcase);
Land_entropy=zeros(length(node1),140,6);
Land_entropy_mix=zeros(length(node1),140,6);
Land_entropy_1=zeros(length(node1),140,6);
Land_entropy_1_mix=zeros(length(node1),140,6);
stage=6;
...
title('Average Entropy for LUAD ');
